...
def weighted_bce_loss(outputs, targets, pos_weight):
    ...
    loss_val = loss_val * (targets * pos_weight + (1 - targets))
    return loss_val.mean()


# Adjust class weights based on class distribution
pos_weight = len(y_resampled) / (2 * sum(y_resampled))
...
best_val_auc = train_model(model, lambda outputs, targets: weighted_bce_loss(
    outputs, targets, pos_weight), optimizer, train_loader, val_loader)
